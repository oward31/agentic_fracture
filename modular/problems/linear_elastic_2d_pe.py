"""Linear elasticity, 2D plane strain — displacement-controlled phase-field.

This is the pattern of `bench_amr.py`. Default BCs:
    bottom: u_y = 0
    top:    u_y = disp_const   (ramped in the driver)
    top-right corner: u_x = 0  (point pin to remove rigid rotation)

Override via `bc_spec_u` if you want different BCs.
"""
...
from ..constitutive import linear_elasticity_2d_pe as const2D_PE
...
def make_linear_elastic_2d_pe_builder(
    mat: Dict,
    markers_spec: List,
    geom: Dict,
    *,
    ...
    """Return a `build_problem(msh) -> P` closure.

    `disp_const` is a shared Constant ramped by the driver; if None, one is
    created internally and exposed as P["disp_const"].
    """
    ...
    def build_problem(msh):
        ...
        em      = const2D_PE.epsilon(u)
        sig     = const2D_PE.sigma(em, mu, lmbda)
        dgd     = const2D_PE.dgd(z, eta)
        psi1    = const2D_PE.energy(em, mu, lmbda)
        I1_und  = const2D_PE.I1_0(em, mu, lmbda, nu)
        SQJ2_u  = const2D_PE.sigmavm(em, mu, lmbda, nu)
        I1_deg  = dgd * I1_und
        SQJ2_d  = dgd * SQJ2_u
        ...
        # ---- Weak forms ------------------------------------------------- #
        R_u = inner(dgd * sig, const2D_PE.epsilon(v)) * dx
        ...
        # ---- AMR indicator + stagger residuals -------------------------- #
...
def _default_bcs_u(msh, V, tag_to_facets, disp_const, pin_point, geom):
    ...
    bot = tag_to_facets[4]  # "bottom" facets
    top = tag_to_facets[3]  # "top" facets
    ...
    # Point-pin u_x at top-right corner.
