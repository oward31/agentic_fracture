"""Linear elasticity, 3D — displacement-controlled phase-field."""
...
from ..constitutive import linear_elasticity_3d as const3D
...
def make_linear_elastic_3d_builder(
    mat: Dict,
    markers_spec: List,
    geom: Dict,
    *,
    ...
    pin_point: Optional[Tuple[float, float, float]] = None,
    ...
    def build_problem(msh):
        ...
        if fresh_bc_spec_u is None:
            bcs_u = _default_bcs_u_3d(msh, V, tag_to_facets, dc, pin_point, geom)
        else:
            bcs_u = build_dirichlet_bcs(V, msh, tag_to_facets, fresh_bc_spec_u)
        bcs_z: list = []

        mu, lmbda, eta = mat["mu"], mat["lmbda"], mat["eta"]
        em      = const3D.epsilon(u)
        sig     = const3D.sigma(em, mu, lmbda)
        dgd     = const3D.dgd(z, eta)
        psi1    = const3D.energy(em, mu, lmbda)
        I1_und  = const3D.I1_0(em, mu, lmbda)
        SQJ2_u  = const3D.sigmavm(em, mu, lmbda)
        I1_deg  = dgd * I1_und
        SQJ2_d  = dgd * SQJ2_u

        ce = drucker_prager_driving_force(z, psi1, I1_deg, SQJ2_d, I1_und, dp_coef)

        R_u = inner(dgd * sig, const3D.epsilon(v)) * dx
...
# -------------------------------------------------------------------------- #
def _default_bcs_u_3d(msh, V, tag_to_facets, disp_const, pin_point, geom):
    """Default 3D BCs: bottom u_y=0, top u_y=disp, one corner pins u_x, u_z."""
    ...
    if pin_point is None and "W" in geom and "L" in geom:
        pin_point = (geom["W"], geom["L"] / 2.0, 0.0)
    if pin_point is not None:
        pin_ents = locate_corner(msh, *pin_point)
        pin_x = fem.locate_dofs_topological(V.sub(0), fdim - 2, pin_ents)
        pin_z = fem.locate_dofs_topological(V.sub(2), fdim - 2, pin_ents)
        bcs.insert(0, fem.dirichletbc(np.array(0.0, dtype=ScalarType), pin_x, V.sub(0)))
        bcs.insert(1, fem.dirichletbc(np.array(0.0, dtype=ScalarType), pin_z, V.sub(2)))
    return bcs
