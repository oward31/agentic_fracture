"""Finite elasticity (Lopez-Pamies Ogden), 3D.

3D analog of the slant solver: CR displacement space, Nitsche/DG
stabilisation, ω-regularised Newton, CG/BoomerAMG for both u and z.

The energy density is fully 3D; no F33 substitution is needed.
"""
...
def make_finite_elastic_3d_builder(
    mat: Dict,
    markers_spec: List,
    geom: Dict,
    *,
    ...
    def build_problem(msh):
        ...
        zero_vec = np.array([0.0, 0.0, 0.0], dtype=PETSc.ScalarType)
        bcs_u  = [fem.dirichletbc(zero_vec, bottom_dofs, V),
                  fem.dirichletbc(r_func, top_dofs)]
        bcs_du = [fem.dirichletbc(zero_vec, bottom_dofs, V),
        ...
        # 3D first Piola for reaction force.
        P_stress = fe.first_piola_3d(u, z, mat)
